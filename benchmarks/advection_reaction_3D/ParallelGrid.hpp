...
public:
  ...
  ParallelGrid(SUNMemoryHelper memhelp, MPI_Comm* comm, const REAL a[],
               ...
               StencilType st, int width = 1, const int npxyz[] = nullptr,
               bool reorder = false)
    : nx(1),
      ny(1),
      nz(1),
      ...
      memhelp(memhelp)
  {
    static_assert((NDIMS >= 1 && NDIMS <= 3),
                  "ParallelGrid NDIMS must be 1, 2 or 3");
  ...
  void AllocateBuffersUpwind()
  {
    ...
    SUNMemoryHelper_Alloc(memhelp, &Wrecv_,
                          sizeof(REAL) * dof * width * nyl * nzl, memoryType(),
                          nullptr);
  ...
  int ExchangeStart(std::function<void(REAL*, REAL*, REAL*, REAL*, REAL*, REAL*)> fill)
  {
    ...
    if (ipW != MPI_PROC_NULL)
    {
      retval = MPI_Irecv(getRecvBuffer("EAST"), dof * nyl * nzl,
                         MPI_SUNREALTYPE, ipW, 1, cart_comm, req);
      assert(retval == MPI_SUCCESS);
    }
    ...
    if (ipE != MPI_PROC_NULL)
    {
      retval = MPI_Irecv(getRecvBuffer("WEST"), dof * nyl * nzl,
                         MPI_SUNREALTYPE, ipE, 0, cart_comm, req + 1);
      assert(retval == MPI_SUCCESS);
    }
    ...
    if (NDIMS >= 2)
    {
      if (ipS != MPI_PROC_NULL)
      {
        retval = MPI_Irecv(getRecvBuffer("NORTH"), dof * nxl * nzl,
                           MPI_SUNREALTYPE, ipS, 3, cart_comm, req + 2);
        assert(retval == MPI_SUCCESS);
      }
      ...
      if (ipN != MPI_PROC_NULL)
      {
        retval = MPI_Irecv(getRecvBuffer("SOUTH"), dof * nxl * nzl,
                           MPI_SUNREALTYPE, ipN, 2, cart_comm, req + 3);
        assert(retval == MPI_SUCCESS);
      }
    }
    ...
    if (NDIMS >= 3)
    {
      if (ipB != MPI_PROC_NULL)
      {
        retval = MPI_Irecv(getRecvBuffer("FRONT"), dof * nxl * nyl,
                           MPI_SUNREALTYPE, ipB, 5, cart_comm, req + 4);
        assert(retval == MPI_SUCCESS);
      }
      ...
      if (ipF != MPI_PROC_NULL)
      {
        retval = MPI_Irecv(getRecvBuffer("BACK"), dof * nxl * nyl,
                           MPI_SUNREALTYPE, ipF, 4, cart_comm, req + 5);
        assert(retval == MPI_SUCCESS);
      }
    }

    // Call user lambda to fill the send buffers
    fill(getSendBuffer("WEST"), getSendBuffer("EAST"), getSendBuffer("SOUTH"),
         getSendBuffer("NORTH"), getSendBuffer("BACK"), getSendBuffer("FRONT"));

    // Send data to neighbors
    if (ipW != MPI_PROC_NULL)
    {
      retval = MPI_Isend(getSendBuffer("EAST"), dof * nyl * nzl,
                         MPI_SUNREALTYPE, ipW, 0, cart_comm, req + 6);
      assert(retval == MPI_SUCCESS);
    }
    ...
    if (ipE != MPI_PROC_NULL)
    {
      retval = MPI_Isend(getSendBuffer("WEST"), dof * nyl * nzl,
                         MPI_SUNREALTYPE, ipE, 1, cart_comm, req + 7);
      assert(retval == MPI_SUCCESS);
    }
    ...
    if (NDIMS >= 2)
    {
      if (ipS != MPI_PROC_NULL)
      {
        retval = MPI_Isend(getSendBuffer("NORTH"), dof * nxl * nzl,
                           MPI_SUNREALTYPE, ipS, 2, cart_comm, req + 8);
        assert(retval == MPI_SUCCESS);
      }
      ...
      if (ipN != MPI_PROC_NULL)
      {
        retval = MPI_Isend(getSendBuffer("SOUTH"), dof * nxl * nzl,
                           MPI_SUNREALTYPE, ipN, 3, cart_comm, req + 9);
        assert(retval == MPI_SUCCESS);
      }
    }
    ...
    if (NDIMS == 3)
    {
      if (ipB != MPI_PROC_NULL)
      {
        retval = MPI_Isend(getSendBuffer("FRONT"), dof * nxl * nyl,
                           MPI_SUNREALTYPE, ipB, 4, cart_comm, req + 10);
        assert(retval == MPI_SUCCESS);
      }
      ...
      if (ipF != MPI_PROC_NULL)
      {
        retval = MPI_Isend(getSendBuffer("BACK"), dof * nxl * nyl,
                           MPI_SUNREALTYPE, ipF, 5, cart_comm, req + 11);
        assert(retval == MPI_SUCCESS);
      }
    }
