...
using raja_xyz_tuple =
  camp::tuple<RAJA::RangeSegment, RAJA::RangeSegment, RAJA::RangeSegment>;
...
static int Advection(realtype t, N_Vector y, N_Vector ydot, void* user_data)
{
  ...
  RAJA::View<realtype, RAJA::Layout<NDIMS + 1>> Yview(GetVecData(y), nxl, nyl,
                                                      nzl, dof);
  RAJA::View<realtype, RAJA::Layout<NDIMS + 1>> dYview(GetVecData(ydot), nxl,
                                                       nyl, nzl, dof);
  ...
  if (c > 0.0)
  {
    if (udata->grid->npx > 1)
    {
      ...
      *  boundaries are west face, south face, front face */
      ...
      RAJA::View<realtype, RAJA::Layout<NDIMS>>
        Yim1jk(udata->grid->getRecvBuffer("WEST"), nyl, nzl,
    ...
    else
    {
      ...
      RAJA::kernel<XYZ_KERNEL_POL>(range,
                                   [=] DEVICE_FUNC(int i, int j, int k)
                                   {
                                     ...
                                     dYview(i, j, k, 0) =
                                       cx * (u_ijk -
                                             Yview(nxl - 1, j, k, 0)); // du/dx
                                     dYview(i, j, k, 1) =
                                       cx * (v_ijk -
                                             Yview(nxl - 1, j, k, 1)); // dv/dx
                                     dYview(i, j, k, 2) =
                                       cx * (w_ijk -
                                             Yview(nxl - 1, j, k, 2)); // dw/dx
                                   });
    }
    ...
    {
      RAJA::View<realtype, RAJA::Layout<NDIMS>>
        Yijm1k(udata->grid->getRecvBuffer("SOUTH"), nxl, nzl,
    ...
    else
    {
      ...
      RAJA::kernel<XYZ_KERNEL_POL>(range,
                                   [=] DEVICE_FUNC(int i, int j, int k)
                                   {
                                     ...
                                     dYview(i, j, k, 0) +=
                                       cy * (u_ijk -
                                             Yview(i, nyl - 1, k, 0)); // du/dy
                                     dYview(i, j, k, 1) +=
                                       cy * (v_ijk -
                                             Yview(i, nyl - 1, k, 1)); // dv/dy
                                     dYview(i, j, k, 2) +=
                                       cy * (w_ijk -
                                             Yview(i, nyl - 1, k, 2)); // dw/dy
                                   });
    }
    ...
    {
      RAJA::View<realtype, RAJA::Layout<NDIMS>>
        Yijkm1(udata->grid->getRecvBuffer("FRONT"), nxl, nyl,
    ...
    else
    {
      ...
      RAJA::kernel<XYZ_KERNEL_POL>(range,
                                   [=] DEVICE_FUNC(int i, int j, int k)
                                   {
                                     ...
                                     dYview(i, j, k, 0) +=
                                       cz * (u_ijk -
                                             Yview(i, j, nzl - 1, 0)); // du/dz
                                     dYview(i, j, k, 1) +=
                                       cz * (v_ijk -
                                             Yview(i, j, nzl - 1, 1)); // dv/dz
                                     dYview(i, j, k, 2) +=
                                       cz * (w_ijk -
                                             Yview(i, j, nzl - 1, 2)); // dw/dz
                                   });
    }
  }
  else if (c < 0.0)
  {
    if (udata->grid->nprocs() != 1)
    {
      ...
      *  boundaries are west face, south face, and front face */
      ...
      RAJA::View<realtype, RAJA::Layout<3>> Yip1jk(udata->grid->getRecvBuffer(
                                                     "EAST"),
                                                   nyl, nzl, dof);
      RAJA::View<realtype, RAJA::Layout<3>> Yijp1k(udata->grid->getRecvBuffer(
                                                     "NORTH"),
                                                   nxl, nzl, dof);
      RAJA::View<realtype, RAJA::Layout<3>> Yijkp1(udata->grid->getRecvBuffer(
                                                     "BACK"),
                                                   nxl, nyl, dof);
    ...
    else
    {
      ...
      RAJA::kernel<XYZ_KERNEL_POL>(range,
                                   [=] DEVICE_FUNC(int i, int j, int k)
                                   {
                                     ...
                                     // grad * u
                                     dYview(i, j, k, 0) =
                                       cz * (u_ijk - Yview(i, j, 0, 0)); // du/dz
                                     dYview(i, j, k, 0) +=
                                       cy * (u_ijk - Yview(i, 0, k, 0)); // du/dy
                                     dYview(i, j, k, 0) +=
                                       cx * (u_ijk - Yview(0, j, k, 0)); // du/dx

                                     // grad * v
                                     dYview(i, j, k, 1) =
                                       cz * (v_ijk - Yview(i, j, 0, 1)); // dv/dz
                                     dYview(i, j, k, 1) +=
                                       cy * (v_ijk - Yview(i, 0, k, 1)); // dv/dy
                                     dYview(i, j, k, 1) +=
                                       cx * (v_ijk - Yview(0, j, k, 1)); // dv/dx

                                     // grad * w
                                     dYview(i, j, k, 2) =
                                       cz * (w_ijk - Yview(i, j, 0, 2)); // dw/dz
                                     dYview(i, j, k, 2) +=
                                       cy * (w_ijk - Yview(i, 0, k, 2)); // dw/dy
                                     dYview(i, j, k, 2) +=
                                       cx * (w_ijk - Yview(0, j, k, 2)); // dw/dx
                                   });
    }
  }
...
/* Solve the linear systems Ax = b where A = I - gamma*dg/dy.
   ...
   dh/dy as dg/dy. */
static int SolveReactionLinSys(N_Vector y, N_Vector x, N_Vector b, realtype gamma,
                               raja_xyz_tuple blocks, UserData* udata)
{
  /* shortcuts */
  int dof, nxl, nyl, nzl;
  ...
  RAJA::View<realtype, RAJA::Layout<NDIMS + 1>> Yview(GetVecData(y), nxl, nyl,
                                                      nzl, dof);
  RAJA::View<realtype, RAJA::Layout<NDIMS + 1>> Bview(GetVecData(b), nxl, nyl,
                                                      nzl, dof);
  RAJA::View<realtype, RAJA::Layout<NDIMS + 1>> Xview(GetVecData(x), nxl, nyl,
                                                      nzl, dof);
  ...
  RAJA::kernel<XYZ_KERNEL_POL>(blocks,
                               [=] DEVICE_FUNC(int i, int j, int k)
                               {
                                 ...
                                 realtype scratch_13 = (-A6 * scratch_12 + A7) /
                                                       (-A3 * scratch_12 + A4);
...
static int PSolve(realtype t, N_Vector y, N_Vector ydot, N_Vector r, N_Vector z,
                  realtype gamma, realtype delta, int lr,
