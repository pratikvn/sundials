...
static int Jn(realtype t, N_Vector y, N_Vector fy, SUNMatrix J, void* user_data,
              N_Vector tmp1, N_Vector tmp2, N_Vector tmp3);
static int MassMatrix(realtype t, SUNMatrix M, void* user_data, N_Vector tmp1,
...
static int check_order(void* arkode_mem, N_Vector y, realtype T0, realtype Tf,
                       ...
                       int order, int rk_type, UserData& udata)
{
  ...
  vector<realtype> hvals = {(Tf - T0) / Nmin,      (Tf - T0) / 2 / Nmin,
...
static realtype r(realtype t) { return (RCONST(0.5) * cos(t)); }

static realtype s(realtype t) { return (sin(t)); }

static realtype rdot(realtype t) { return (-RCONST(0.5) * sin(t)); }

static realtype sdot(realtype t) { return (cos(t)); }

static realtype utrue(realtype t) { return (SUNRsqrt(ONE + r(t))); }

static realtype vtrue(realtype t) { return (SUNRsqrt(TWO + s(t))); }
