...
static int f(realtype t, N_Vector y, N_Vector ydot, void* user_data)
{
  ...
  for (i = 1; i < N - 1; i++)
  {
    ...
    dYdata[IDX(i, 0)] = (ul - RCONST(2.0) * u + ur) * uconst + a -
                        (w + RCONST(1.0)) * u + v * u * u;

    /* Fill in ODE RHS for v */
    dYdata[IDX(i, 1)] = (vl - RCONST(2.0) * v + vr) * vconst + w * u - v * u * u;

    /* Fill in ODE RHS for w */
    dYdata[IDX(i, 2)] = (wl - RCONST(2.0) * w + wr) * wconst + (b - w) / ep -
                        w * u;
  }
...
static int LaplaceMatrix(realtype c, SUNMatrix Jac, UserData udata)
{
  ...
  for (i = 1; i < N - 1; i++)
  {
    ...
    SM_ELEMENT_B(Jac, IDX(i, 0), IDX(i, 0)) += -c * RCONST(2.0) * udata->du /
                                               dx / dx;
    SM_ELEMENT_B(Jac, IDX(i, 1), IDX(i, 1)) += -c * RCONST(2.0) * udata->dv /
                                               dx / dx;
    SM_ELEMENT_B(Jac, IDX(i, 2), IDX(i, 2)) += -c * RCONST(2.0) * udata->dw /
                                               dx / dx;
    SM_ELEMENT_B(Jac, IDX(i, 0), IDX(i + 1, 0)) += c * udata->du / dx / dx;
    SM_ELEMENT_B(Jac, IDX(i, 1), IDX(i + 1, 1)) += c * udata->dv / dx / dx;
    SM_ELEMENT_B(Jac, IDX(i, 2), IDX(i + 1, 2)) += c * udata->dw / dx / dx;
...
static int ReactionJac(realtype c, N_Vector y, SUNMatrix Jac, UserData udata)
{
  ...
  for (i = 1; i < N - 1; i++)
  {
    ...
    SM_ELEMENT_B(Jac, IDX(i, 0),
                 IDX(i, 0)) += c * (RCONST(2.0) * u * v - (w + RCONST(1.0)));
