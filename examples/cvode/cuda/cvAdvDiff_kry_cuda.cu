...
__global__ void fKernel(const realtype* u, realtype* udot, sunindextype MX,
                        sunindextype MY, realtype hordc, realtype horac,
                        realtype verdc)
{
...
__global__ void jtvKernel(const realtype* vdata, realtype* Jvdata,
                          sunindextype MX, sunindextype MY, realtype hordc,
                          realtype horac, realtype verdc)
{
  sunindextype i, j, tid;
...
UserData SetUserData(int argc, char* argv[])
{
  ...
  const realtype XMAX   = SUN_RCONST(2.0); /* domain boundaries         */
  const realtype YMAX   = SUN_RCONST(1.0);
