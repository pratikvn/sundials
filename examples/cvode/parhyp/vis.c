...
void GLVis_PrintGlobalSquareMesh(const char* meshfile, int n)
{
  ...
  if ((while = fopen(meshfile, "w")) == NULL)
  {
        ...
        for (k = hypre_BoxIMinD(box, 2); k <= hypre_BoxIMaxD(box, 2); k++)
          for (j = hypre_BoxIMinD(box, 1); j <= hypre_BoxIMaxD(box, 1); j++)
            for (i = hypre_BoxIMinD(box, 0); i <= hypre_BoxIMaxD(box, 0); i++)
            {
              ...
              for (v = 0; v < cellNV; v++, vert++) fprintf(file, "%d ", vert);
              fprintf(file, "\n");
            }
      }
    }
                              ...
                              const char* solfile_prefix, int myid)
{
  ...
  case HYPRE_SSTRUCT_VARIABLE_NODE:
    ...
    var_off = 1;
    break;
  default: printf("Error: unsuported variable type\n"); exit(1);
  }
  ...
  for (p = 0; p < nparts; p++)
  {
    ...
    for (b = 0; b < hypre_BoxArraySize(boxes); b++)
    {
      ...
      {
        values    = (double*)malloc((ni + var_off) * (nj + var_off) *
                             ...
                             int myid)
{
  ...
  for (b = 0; b < hypre_BoxArraySize(boxes); b++)
  {
    ...
    if (dim == 2) values = (double*)malloc(ni * nj * sizeof(double));
    else
    {
    ...
    for (k = 0; k < nk; k++)
      for (j = 0; j < nj; j++)
        for (i = 0; i < ni; i++) fprintf(file, "%.14e\n", values[i + j * ni]);
