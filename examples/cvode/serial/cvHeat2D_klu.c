...
int main(void)
{
  ...
  */
  ...
  if (MGRID >= 4) { retval = CVodeSetJacFn(cvode_mem, jacHeat); }
  /* special case MGRID=3  */
  else if (MGRID == 3) { retval = CVodeSetJacFn(cvode_mem, jacHeat3); }
                   ...
                   N_Vector tempv3)
{
  ...
  for (i = 0; i < MGRID - 4; i++)
  {
    ...
    rowvals[2 * MGRID + 4 * (MGRID - 2) + repeat + 3] = MGRID + (MGRID - 2) +
                                                        2 + MGRID * i + 1;
    rowvals[2 * MGRID + 4 * (MGRID - 2) + repeat + 4] = MGRID + (MGRID - 2) + 2 +
                                                        MGRID * i + 2; //*this
...
static int SetInitialProfile(UserData data, N_Vector uu, N_Vector res)
{
  ...
  for (j = 0; j < mm; j++)
  {
    ...
    {
      loc = offset + i;
      if (j == 0 || j == mm1 || i == 0 || i == mm1) { udata[loc] = BVAL; }
    }
  }
