...
__global__ void resHeatKernel(const realtype* uu, const realtype* up,
                              realtype* rr, sunindextype mm, realtype coeff)
{
...
__global__ void PsetupHeatKernel(realtype* ppv, sunindextype mm, realtype c_j,
                                 realtype coeff)
{
  sunindextype i, j, tid;
...
__global__ void setInitHeatKernel(realtype* up, sunindextype mm)
{
  sunindextype i, j, tid;
...
static int SetInitialProfile(UserData data, N_Vector uu, N_Vector up, N_Vector res)
{
  ...
  for (j = 0; j < mm; j++)
  {
    ...
    {
      xfact             = data->dx * i;
      udata[mm * j + i] = RCONST(16.0) * xfact * (ONE - xfact) * yfact *
                          (ONE - yfact);
    }
  }
