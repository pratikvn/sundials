...
static int reslocal(realtype tt, N_Vector uu, N_Vector up, N_Vector res,
                    void* user_data);
...
static int BRecvPost(MPI_Comm comm, MPI_Request request[], int thispe, int ixsub,
                     ...
                     sunindextype mysub, realtype* host_recv_buff);

static int BRecvWait(MPI_Request request[], int ixsub, int jysub, int npex,
                     int npey, sunindextype mxsub, sunindextype mysub,
                     realtype* uext, const realtype* host_recv_buff,
                     realtype* dev_recv_buff);
...
__global__ void PsetupHeatKernel(realtype* ppv, sunindextype mx,
                                 sunindextype my, sunindextype ibc,
                                 sunindextype jbc, sunindextype i0,
                                 sunindextype j0, realtype pelinv)
{
...
__global__ void CopyLocalToExtendedArray(const realtype* uuv, realtype* uext,
...
__global__ void LocalResidualKernel(const realtype* uext, const realtype* upv,
                                    realtype* resv, sunindextype mx,
                                    sunindextype my, sunindextype ibc,
                                    sunindextype jbc, sunindextype i0,
                                    sunindextype j0, realtype coeffx,
                                    realtype coeffy, realtype coeffxy)
{
  sunindextype i, j, tid;
...
__global__ void CopyToBottomBuffer(const realtype* uarray, realtype* bufbottom,
                                   sunindextype mx)
{
  sunindextype tid;
...
__global__ void CopyToTopBuffer(const realtype* uarray, realtype* buftop,
                                sunindextype mx, sunindextype my)
{
  sunindextype tid;
...
__global__ void CopyToLeftBuffer(const realtype* uarray, realtype* bufleft,
                                 sunindextype mx, sunindextype my)
{
  sunindextype tid;
...
__global__ void CopyToRightBuffer(const realtype* uarray, realtype* bufright,
                                  sunindextype mx, sunindextype my)
{
  sunindextype tid;
...
__global__ void CopyFromBottomBuffer(const realtype* bufbottom, realtype* uext,
                                     sunindextype mx)
{
  sunindextype tid;
...
__global__ void CopyFromTopBuffer(const realtype* buftop, realtype* uext,
                                  sunindextype mx, sunindextype my)
{
  sunindextype tid;
...
__global__ void CopyFromLeftBuffer(const realtype* bufleft, realtype* uext,
                                   sunindextype mx, sunindextype my)
{
  sunindextype tid;
...
__global__ void CopyFromRightBuffer(const realtype* bufright, realtype* uext,
                                    sunindextype mx, sunindextype my)
{
  sunindextype tid;
                 ...
                 realtype* host_send_buff)
{
  ...
  if (jysub != npey - 1)
  {
    ...
    // Copy buffer to the host
    err = cudaMemcpy(h_buftop, d_buftop, mxsub * sizeof(realtype),
                     cudaMemcpyDeviceToHost);
    if (err != cudaSuccess)
    {
  ...
  if (ixsub != 0)
  {
    ...
    // Copy buffer to the host
    err = cudaMemcpy(h_bufleft, d_bufleft, mysub * sizeof(realtype),
                     cudaMemcpyDeviceToHost);
    if (err != cudaSuccess)
    {
  ...
  if (ixsub != npex - 1)
  {
    ...
    // Copy buffer to the host
    err = cudaMemcpy(h_bufright, d_bufright, mysub * sizeof(realtype),
                     cudaMemcpyDeviceToHost);
    if (err != cudaSuccess)
    {
                     ...
                     realtype* dev_recv_buff)
{
  ...
  if (jysub != 0)
  {
    ...
    err = cudaMemcpy(d_bufbottom, h_bufbottom, mxsub * sizeof(realtype),
  ...
  if (jysub != npey - 1)
  {
    ...
    /* Copy the buffer from the host to the device */
    err = cudaMemcpy(d_buftop, h_buftop, mxsub * sizeof(realtype),
                     cudaMemcpyHostToDevice);
    if (err != cudaSuccess)
    {
      printf("Copy from host to device failed ... \n");
  ...
  if (ixsub != 0)
  {
    ...
    /* Copy the buffer from the host to the device */
    err = cudaMemcpy(d_bufleft, h_bufleft, mysub * sizeof(realtype),
                     cudaMemcpyHostToDevice);
    if (err != cudaSuccess)
    {
      printf("Copy from host to device failed ... \n");
  ...
  if (ixsub != npex - 1)
  {
    ...
    /* Copy the buffer from the host to the device */
    err = cudaMemcpy(d_bufright, h_bufright, mysub * sizeof(realtype),
                     cudaMemcpyHostToDevice);
    if (err != cudaSuccess)
    {
      printf("Copy from host to device failed ... \n");
...
static int AllocUserData(int thispe, MPI_Comm comm, N_Vector uu, UserData data)
{
  ...
  err = cudaMalloc((void**)&data->uext,
                   (mxsub + 2) * (mysub + 2) * sizeof(realtype));
  if (err != cudaSuccess)
  {
