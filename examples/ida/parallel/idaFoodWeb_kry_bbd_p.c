...
static int resweb(realtype tt, N_Vector cc, N_Vector cp, N_Vector rr,
                  void* user_data);

static int reslocal(sunindextype Nlocal, realtype tt, N_Vector cc, N_Vector cp,
                    N_Vector res, void* user_data);

static int rescomm(sunindextype Nlocal, realtype tt, N_Vector cc, N_Vector cp,
                   void* user_data);
  ...
  for (jy = 0; jy < mysub; jy++)
  {
    ...
    for (ix = 0; ix < mxsub; ix++)
    {
      ...
      for (is = 0; is < NUM_SPECIES; is++)
      {
        if (is < np)
        {
          cxy[is]  = RCONST(10.0) + (realtype)(is + 1) * xyfactor;
          idxy[is] = ONE;
        }
        else
        {
...
{
  realtype *cdata, *ratesxy, *cpxy, *resxy, xx, yy, dcyli, dcyui, dcxli, dcxui;
