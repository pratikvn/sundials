                 ...
                 sunrealtype* rout, long int* ipar, sunrealtype* rpar, int* ier)
{
  ...
  *ier = 0;
  ...
  if (F2C_ARKODE_vec->ops->nvgetarraypointer == NULL)
  {
    *ier = -1;
    fprintf(stderr,
            "Error: getarraypointer vector operation is not implemented.\n\n");
    return;
  }
  if (F2C_ARKODE_vec->ops->nvsetarraypointer == NULL)
  {
    *ier = -1;
    fprintf(stderr,
            "Error: setarraypointer vector operation is not implemented.\n\n");
    return;
  }
  if (F2C_ARKODE_vec->ops->nvcloneempty == NULL)
  {
    *ier = -1;
    fprintf(stderr,
            "Error: cloneempty vector operation is not implemented.\n\n");
    return;
  }
                 ...
                 sunrealtype* rtol, sunrealtype* atol, int* ier)
{
  ...
  *ier = 0;
  ...
  {
  case 0: /* purely implicit */
    *ier = ARKStepReInit(ARK_arkodemem, NULL, FARKfi, *t0, F2C_ARKODE_vec);
    break;
  case 1: /* purely explicit */
    *ier = ARKStepReInit(ARK_arkodemem, FARKfe, NULL, *t0, F2C_ARKODE_vec);
    break;
  case 2: /* imex */
    *ier = ARKStepReInit(ARK_arkodemem, FARKfe, FARKfi, *t0, F2C_ARKODE_vec);
    break;
  }
                 ...
                 int* itol, sunrealtype* rtol, sunrealtype* atol, int* ier)
{
  ...
  *ier = ARKStepResize(ARK_arkodemem, F2C_ARKODE_vec, *hscale, *t0, NULL, NULL);
...
void FARK_SETARKTABLES(int* s, int* q, int* p, sunrealtype* ci, sunrealtype* ce,
                       sunrealtype* Ai, sunrealtype* Ae, sunrealtype* bi,
                       sunrealtype* be, sunrealtype* b2i, sunrealtype* b2e,
                       int* ier)
{
...
/* ---DEPRECATED---
   ...
   farkode.h for further details */
void FARK_DLSINIT(int* ier) { FARK_LSINIT(ier); }
...
/* ---DEPRECATED---
   Fortran interface to C routine ARKStepSetLinearSolver; see
   farkode.h for further details */
void FARK_SPILSINIT(int* ier) { FARK_LSINIT(ier); }
...
/* Fortran interface to C routine ARKStepEvolve (the main integrator)
   ...
   further details */
void FARK_ARKODE(sunrealtype* tout, sunrealtype* t, sunrealtype* y, int* itask,
                 int* ier)
{
  ...
  *ier = ARKStepEvolve(ARK_arkodemem, *tout, F2C_ARKODE_vec, t, *itask);
  ...
  ARKStepGetWorkSpace(ARK_arkodemem, &ARK_iout[0],        /* LENRW   */
                      &ARK_iout[1]);                      /* LENIW   */
  ARKStepGetStepStats(ARK_arkodemem, &ARK_iout[2],        /* NST     */
                      &ARK_rout[0],                       /* H0U     */
                      &ARK_rout[1],                       /* HU      */
                      &ARK_rout[2],                       /* HCUR    */
                      &ARK_rout[3]);                      /* TCUR    */
  ARKStepGetTimestepperStats(ARK_arkodemem, &ARK_iout[3], /* NST_STB */
                             &ARK_iout[4],                /* NST_ACC */
                             &ARK_iout[5],                /* NST_ATT */
                             &ARK_iout[6],                /* NFE     */
                             &ARK_iout[7],                /* NFI     */
                             &ARK_iout[8],                /* NSETUPS */
                             &ARK_iout[9]);               /* NETF    */
  ARKStepGetTolScaleFactor(ARK_arkodemem, &ARK_rout[4]);  /* TOLSFAC */
  ARKStepGetNonlinSolvStats(ARK_arkodemem, &ARK_iout[10], /* NNI     */
                            &ARK_iout[11]);               /* NCFN    */
  ...
  if (ARK_ls)
  {
    ARKStepGetLinWorkSpace(ARK_arkodemem, &ARK_iout[13],
                           &ARK_iout[14]);               /* LENRWLS, LENIWLS */
  ...
  if (ARK_mass_ls)
  {
    ARKStepGetMassWorkSpace(ARK_arkodemem, &ARK_iout[24],
                            &ARK_iout[25]);               /* LENRWMS, LENIWMS */
  ...
  if (ARK_constr) { ARKStepGetNumConstrFails(ARK_arkodemem, &ARK_iout[35]); }
  return;
}
...
void FARK_FREE()
{
  ...
  if (ark_mem->user_data) free(ark_mem->user_data);
...
int FARKfe(sunrealtype t, N_Vector y, N_Vector ydot, void* user_data)
{
  ...
  FARK_EXP_FUN(&t, ydata, dydata, ARK_userdata->ipar, ARK_userdata->rpar, &ier);
  return (ier);
}
...
int FARKfi(sunrealtype t, N_Vector y, N_Vector ydot, void* user_data)
{
  ...
  FARK_IMP_FUN(&t, ydata, dydata, ARK_userdata->ipar, ARK_userdata->rpar, &ier);
  return (ier);
}
