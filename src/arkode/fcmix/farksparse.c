...
extern void FARK_SPJAC(sunrealtype* T, sunrealtype* Y, sunrealtype* FY,
...
void FARK_SPARSESETJAC(int* ier)
{
  ...
  arkProcessError((ARKodeMem)ARK_arkodemem, ARK_ILL_INPUT, "ARKODE",
                  "FARKSPARSESETJAC", "Sparse Fortran users must configure SUNDIALS with 64-bit integers.");
  *ier = 1;
#else
                  ...
                  N_Vector vtemp3)
{
  ...
  FARK_SPJAC(&t, ydata, fydata, &NP, &NNZ, Jdata, indexvals, indexptrs, &h,
             ARK_userdata->ipar, ARK_userdata->rpar, v1data, v2data, v3data,
             &ier);
  return (ier);
}
