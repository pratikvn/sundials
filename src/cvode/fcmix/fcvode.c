                ...
                sunrealtype* rout, long int* ipar, sunrealtype* rpar, int* ier)
{
  ...
  *ier = CVodeInit(CV_cvodemem, FCVf, *t0, F2C_CVODE_vec);
  ...
  switch (*iatol)
  {
  case 1: *ier = CVodeSStolerances(CV_cvodemem, *rtol, *atol); break;
  case 2:
    Vatol = NULL;
...
/* ---DEPRECATED--- */
void FCV_DLSINIT(int* ier) { FCV_LSINIT(ier); }

/***************************************************************************/

/* ---DEPRECATED--- */
void FCV_SPILSINIT(int* ier) { FCV_LSINIT(ier); }
               ...
               int* ier)
{
  ...
  *ier = CVode(CV_cvodemem, *tout, F2C_CVODE_vec, t, *itask);
  ...
  CVodeGetWorkSpace(CV_cvodemem, &CV_iout[0],       /* LENRW   */
                    &CV_iout[1]);                   /* LENIW   */
  CVodeGetIntegratorStats(CV_cvodemem, &CV_iout[2], /* NST     */
                          &CV_iout[3],              /* NFE     */
                          &CV_iout[7],              /* NSETUPS */
                          &CV_iout[4],              /* NETF    */
                          &qu,                      /* QU      */
  ...
  CVodeGetNonlinSolvStats(CV_cvodemem, &CV_iout[6],       /* NNI     */
                          &CV_iout[5]);                   /* NCFN    */
  ...
  {
  case CV_LS_STD:
    CVodeGetLinWorkSpace(CV_cvodemem, &CV_iout[12],
                         &CV_iout[13]);                 /* LENRWLS,LENIWLS */
    ...
    break;
  case CV_LS_DIAG:
    CVDiagGetWorkSpace(CV_cvodemem, &CV_iout[12],
                       &CV_iout[13]);                /* LENRWLS,LENIWLS */
