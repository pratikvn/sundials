...
void FIDA_SPARSESETJAC(int* ier)
{
  ...
  IDAProcessError((IDAMem)IDA_idamem, IDA_ILL_INPUT, "IDA",
                  "FIDASPARSESETJAC", "Sparse Fortran users must configure SUNDIALS with 64-bit integers.");
  *ier = 1;
#else
                  ...
                  N_Vector vtemp2, N_Vector vtemp3)
{
  ...
  FIDA_SPJAC(&t, &cj, ydata, ypdata, rdata, &NP, &NNZ, Jdata, indexvals,
             indexptrs, &h, IDA_userdata->ipar, IDA_userdata->rpar, v1data,
             v2data, v3data, &ier);
  return (ier);
}
