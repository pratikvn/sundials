...
int idaLsPerf(IDAMem IDA_mem, int perftask)
{
  ...
  if (lcfn)
  {
    IDAProcessError(IDA_mem, IDA_WARNING, __LINE__, __func__, __FILE__, MSG_LS_CFN_WARN);
  }
  if (lcfl)
  {
    IDAProcessError(IDA_mem, IDA_WARNING, __LINE__, __func__, __FILE__, MSG_LS_CFL_WARN);
  }
  return (0);
}
