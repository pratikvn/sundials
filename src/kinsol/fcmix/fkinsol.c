...
void FKIN_CREATE(int* ier)
{
  *ier = 0;
      ...
      (F2C_KINSOL_vec->ops->nvsetarraypointer == NULL))
  {
    *ier = -1;
    fprintf(stderr,
            "FKINCREATE: A required vector operation is not implemented.\n\n");
    return;
  }
...
void FKIN_LSINIT(int* ier)
{
  ...
  *ier = KINSetLinearSolver(KIN_kinmem, F2C_KINSOL_linsol, F2C_KINSOL_matrix);
  return;
}
...
void FKIN_DLSINIT(int* ier) { FKIN_LSINIT(ier); }

/*------------------------------------------------------------------
  Function : FKIN_SPILSINIT -- DEPRECATED
  ------------------------------------------------------------------*/

void FKIN_SPILSINIT(int* ier) { FKIN_LSINIT(ier); }
...
void FKIN_FREE(void)
{
  ...
  if (kin_mem->kin_user_data) free(kin_mem->kin_user_data);
