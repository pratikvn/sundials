...
void FKIN_SPARSESETJAC(int* ier)
{
  ...
  KINProcessError((KINMem)KIN_kinmem, KIN_ILL_INPUT, "KIN",
                  "FKINSPARSESETJAC", "Sparse Fortran users must configure SUNDIALS with 64-bit integers.");
  *ier = 1;
#else
...
int FKINSparseJac(N_Vector y, N_Vector fy, SUNMatrix J, void* user_data,
                  N_Vector vtemp1, N_Vector vtemp2)
{
  ...
  FKIN_SPJAC(ydata, fydata, &NP, &NNZ, Jdata, indexvals, indexptrs, v1data,
             v2data, &ier);
  return (ier);
}
