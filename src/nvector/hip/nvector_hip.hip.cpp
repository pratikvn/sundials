...
#define NVEC_HIP_HBUFFERp(x) \
  ((realtype*)NVEC_HIP_PRIVATE(x)->reduce_buffer_host->ptr)
#define NVEC_HIP_DBUFFERp(x) \
  ((realtype*)NVEC_HIP_PRIVATE(x)->reduce_buffer_dev->ptr)
...
N_Vector N_VNewWithMemHelp_Hip(sunindextype length, booleantype use_managed_mem,
                                    ...
                                    N_Vector* X, N_Vector** Y, N_Vector** Z)
{
  ...
  if (!SUNDIALS_HIP_VERIFY(err)) { return (-1); }
  err = hipMemcpy(d_Yd, h_Yd, nsum * nvec * sizeof(realtype*),
  ...
  if (!SUNDIALS_HIP_VERIFY(err)) { return (-1); }
  err = hipMemcpy(d_Zd, h_Zd, nsum * nvec * sizeof(realtype*),
                  hipMemcpyHostToDevice);
  if (!SUNDIALS_HIP_VERIFY(err)) { return (-1); }
                                        ...
                                        N_Vector** X, N_Vector* Z)
{
  ...
  if (!SUNDIALS_HIP_VERIFY(err)) { return (-1); }
  err = hipMemcpy(d_Xd, h_Xd, nsum * nvec * sizeof(realtype*),
                  hipMemcpyHostToDevice);
  if (!SUNDIALS_HIP_VERIFY(err)) { return (-1); }
