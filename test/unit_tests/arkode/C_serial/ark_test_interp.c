  ...
  for (ideg = 0; ideg <= ARK_INTERP_MAX_DEGREE; ideg++)
  {
    ...
    for (ih = 0; ih < NHVALS; ih++)
    {
      ...
      for (itest = 0; itest < nttest; itest++)
      {
        ...
        /*   y */
        NV_Ith_S(yerr,
                 0) = SUNRabs(SIN(RCONST(2.0) * t_test) - NV_Ith_S(ytest, 0));
        NV_Ith_S(yerr,
                 1) = SUNRabs(COS(RCONST(3.0) * t_test) - NV_Ith_S(ytest, 1));
        /*   dy */
        NV_Ith_S(dyerr, 0) = SUNRabs(RCONST(2.0) * COS(RCONST(2.0) * t_test) -
                                     NV_Ith_S(dytest, 0));
        NV_Ith_S(dyerr, 1) = SUNRabs(-RCONST(3.0) * SIN(RCONST(3.0) * t_test) -
                                     NV_Ith_S(dytest, 1));
        /*   d2y */
        NV_Ith_S(d2yerr, 0) = SUNRabs(-RCONST(4.0) * SIN(RCONST(2.0) * t_test) -
                                      NV_Ith_S(d2ytest, 0));
        NV_Ith_S(d2yerr, 1) = SUNRabs(-RCONST(9.0) * COS(RCONST(3.0) * t_test) -
                                      NV_Ith_S(d2ytest, 1));
        /*   d3y */
        NV_Ith_S(d3yerr, 0) = SUNRabs(-RCONST(8.0) * COS(RCONST(2.0) * t_test) -
                                      NV_Ith_S(d3ytest, 0));
        NV_Ith_S(d3yerr, 1) = SUNRabs(RCONST(27.0) * SIN(RCONST(3.0) * t_test) -
                                      NV_Ith_S(d3ytest, 1));
        /*   d4y */
        NV_Ith_S(d4yerr, 0) = SUNRabs(RCONST(16.0) * SIN(RCONST(2.0) * t_test) -
                                      NV_Ith_S(d4ytest, 0));
        NV_Ith_S(d4yerr, 1) = SUNRabs(RCONST(81.0) * COS(RCONST(3.0) * t_test) -
                                      NV_Ith_S(d4ytest, 1));
        /*   d5y */
        NV_Ith_S(d5yerr, 0) = SUNRabs(RCONST(32.0) * COS(RCONST(2.0) * t_test) -
                                      NV_Ith_S(d5ytest, 0));
        NV_Ith_S(d5yerr, 1) = SUNRabs(
          -RCONST(243.0) * SIN(RCONST(3.0) * t_test) - NV_Ith_S(d5ytest, 1));
...
static int f(realtype t, N_Vector y, N_Vector ydot, void* user_data)
{
  ...
  NV_Ith_S(ydot, 0) = (*lambda) * (NV_Ith_S(y, 0) - SIN(RCONST(2.0) * t)) +
                      NV_Ith_S(y, 1) - COS(RCONST(3.0) * t) +
                      RCONST(2.0) * COS(RCONST(2.0) * t);
