...
int main(void)
{
  ...
  realtype T0     = SUN_RCONST(0.0);
  realtype dTout  = SUN_RCONST(0.1);
  realtype lambda = SUN_RCONST(-25.0);
  realtype rtol   = SUN_RCONST(1e-3);
  realtype atol   = SUN_RCONST(1e-6);
...
/* analytical solution */
static realtype ytrue(realtype t) { return (atan(t)); }
